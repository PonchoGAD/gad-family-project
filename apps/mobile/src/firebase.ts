...
// Без прямого импорта 'firebase/auth/react-native', чтобы не ловить TS-ошибку.
const auth = (() => {
  try {
...
// --- Единая точка вызова функций (имена совпадают с CF) ---
export const fn = {
  geo: {
    ping: call<{ lat: number; lng: number; acc: number | null }, { ok: true }>("geo_ping"),
    places: call<{ q: string }, { ok: true; items: any[] }>("geo_places"),
    history: call<{ from?: number; to?: number }, { ok: true; items: any[] }>("geo_history"),
    setPlace: call<{
      placeId: string;
      type: string;
      title: string;
  ...
  // Легаси-алиасы
  locationPing: call<{ lat: number; lng: number; acc: number | null }, { ok: true }>("geo_ping"),
  getLocationHistory: call<{ targetUid?: string; fromISO?: string; toISO?: string }, { ok: true; items: any[] }>("geo_history"),
  setPlace: call<{ placeId: string; type: string; title: string; center: [number, number]; radiusM: number }, { ok: true }>("geo_setPlace"),

  // Step Engine
  stepEngineRunNow: call<unknown, { ok: boolean; processed: number; date: string }>("stepEngineRunNow"),
};

export { app, auth, db, functions, storage, getFirestore, getFunctions, httpsCallable };
